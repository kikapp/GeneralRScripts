...
n.sum <-  function(dlist) {
	...
	for ( level in rownames( summary(dlist) ) ) {
		...
		#temp[[level]] <- cbind(c("N", freq), c("%", prop*100 ) )
...
aggregate.cont.sum <- function(list.of.data, list.of.varnames, to.round) {
  list.of.tables <- llply(list.of.varnames, function(var.info, data.list) {
    return(cont.sum(data.list, var.info, to.round))
  }, list.of.data)
...
cont.sum <- function(dlist, varname.array, to.round) {
	temp <- list()
...
#dlist <- culled.groups
#varname.array <- cat.vars.to.summarize[[3]]
...
cat.sum <- function(dlist, varname.array) {
	...
	colnames(to.return) <- rep(rownames( summary(dlist)), rep(2, nrow(summary(dlist) ) ) ) 
	...
	to.return <- cbind(rownames(to.return), to.return)
	to.return <- rbind(c(tablename,rep(rownames( summary(dlist)), rep(2, nrow(summary(dlist) ) ) )), 
					   to.return,
					   rep("", ncol(to.return)))
	return( to.return )
}
...
#
# varname.array <- cat.vars.to.summarize[[5]]
...
#   print(to.table)
  ...
  if (ncol(table(to.table[,1],to.table[,2])) == 1) {
    pval <- "NA"
  }
  return(pval)
}
				  
aggregate.cat.sum <- function(list.of.data, list.of.varnames) {
	list.of.tables <- llply(list.of.varnames, function(var.info, data.list) {
